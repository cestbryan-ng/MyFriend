...
public class Page1VideoController implements Initializable {
    ...
    private void sendAudio() {
        try {
            ...
            // CORRECTION 1: Pas de buffer pour réduire latence (comme dans l'appel audio)
            micro.open(format);
            micro.start();
            ...
            while (encours && !Thread.currentThread().isInterrupted()) {
                ...
                if (bytesRead > 0) {
                    // CORRECTION 2: Pas de synchronized ni flush (comme dans l'appel audio)
    ...
    private void receiveAudio() {
        try (SourceDataLine speaker = AudioSystem.getSourceDataLine(format)) {
            // CORRECTION 3: Pas de buffer pour réduire latence (comme dans l'appel audio)
            speaker.open(format);
            speaker.start();
            ...
            while (encours && !Thread.currentThread().isInterrupted()) {
                try {
                    ...
                    // CORRECTION 5: Timeout = déconnexion détectée (comme dans l'appel audio)
